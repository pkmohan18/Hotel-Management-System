...
async function bookroom(roomNo) {
  if (confirm(`Do you want to book Room : ${roomNo} ?`)) {
    let resp = await fetch("http://localhost:8080/HotelManagementSystemMVC/room/getByNo?num="+roomNo);
    ...
    let bookedRooms = {
      ...
      let response = await fetch("http://localhost:8080/HotelManagementSystemMVC/bookedRoom/save",{
            method :'POST',
            headers : {
...
async function delbroom(id){
  let resp = await fetch("http://localhost:8080/HotelManagementSystemMVC/bookedRoom/delete?id="+id,{
        method : "DELETE"
    });
